...
//             if (!manifestData) throw new Error("Failed to load manifest.json");
//             buildTreeView(manifestData);
//         })
//         .catch(function(error) {
//             console.error("Error loading manifest.json:", error);
//         });
...
// // ===== GRAPH BUILD FUNCTION =====
// function buildGraph(manifestData) {
//     const nodes = [];
//     const links = [];
//     const nodeMap = new Map();
...
//     // Create Platform and Difficulty nodes first
//     const platforms = [...new Set(manifestData.machines.map(m => m.platform.toUpperCase()))];
//     const difficulties = [...new Set(manifestData.machines.map(m => m.difficulty))]
//         .filter(diff => diff);
...
//     // Create Machine nodes and Links
//     manifestData.machines.forEach(machine => {
//         const machineNode = { 
//             id: machine.title, 
//             type: "machine", 
//             radius: 7, 
...
// // ===== TREE VIEW FUNCTION =====
// function buildTreeView(manifestData) {
//     const treeContainer = d3.select("#tree-container");
//     treeContainer.html("");
...
//         if (hasDifficulties) {
//             difficultyKeys.forEach((difficulty, difficultyIndex) => {
//                 if (difficulty === "_no_difficulty") return; // Skip no-difficulty group here
...
//         // Handle machines without difficulties (directly under platform)
//         if (platforms[platform]["_no_difficulty"]) {
//             const machines = platforms[platform]["_no_difficulty"];
//             const isLastGroup = !hasDifficulties;
...
//             machines.forEach((machine, machineIndex) => {
//                 const isLastMachine = machineIndex === machines.length - 1;
//                 const connector = hasDifficulties ? '└── ' : ''; // Only show connector if there are other difficulties
...
//         treeContainer.append("div")
//             .attr("class", "tree-line")
//             .text("No machines found. Add some to your manifest.json!");
//     }
// }

// // ===== INITIALIZE =====
// // Load tree view data on initial load
// loadTreeData();
